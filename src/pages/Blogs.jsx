...
const Blogs = () => {
  ...
  const handleOptionClick = (value) => {
    ...
    setData(blogs.filter(blog=>blog.company.toLowerCase().indexOf(value.toLowerCase()) !== -1))
  ...
  useEffect(() => {
    ...
    if (data.length > 0) {
      let comp = [];
      data.map((blog) => {
        if (blog.tags.includes("Interview Experiences")) {
  ...
  const TopicCard = ({ title }) => {
    return (
      <div
        className="sm:w-[16rem] w-[10rem] text-[white] my-5 flex justify-center items-center sm:px-6 px-4 py-2 rounded-lg shadow-xl cursor-pointer bg-[#E97500] border-[#E97500] hover:border-[#E97500] hover:bg-[#202128] hover:text-[white]"
          ...
          {activeFilter === "Interview Experiences" && (
            <div className="company-input flex flex-col ease-in-out mb-12 justify-center items-center z-10">
              <input
                ...
                className="text-black w-[70%] text-center rounded-[8px]"
              />
