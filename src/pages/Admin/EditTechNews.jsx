...
const EditTechNews = () => {
	...
	const { editTechNew, getTechNew, techNew } = useContext(codeContext);
	useEffect(() => {
		getTechNew(id);
	}, []);
	useEffect(() => {
	...
	const handleUpload = () => {
		if (
			...
			swal({
				title: "Do you stil want to edit the Tech News ?",
				icon: "warning",
				buttons: true,
			}).then((res) => {
				if (res) {
					...
					swal({
						title: "Success",
						text: "Tech News edited successfully",
						icon: "success",
					});
