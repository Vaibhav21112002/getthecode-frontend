  ...
  const TableComponent = ({ item }) => {
    return (
      <tr className="bg-white dark:bg-gray-800 text-[0.76rem]">
        <th
          scope="row"
          className="py-4 px-6 font-medium text-center text-gray-900 whitespace-nowrap dark:text-white hover:text-blue-600 dark:hover:text-blue-500 cursor-pointer"
        ...
        <td className="py-4 px-6 text-right">
          <button className="font-medium text-blue-600 dark:text-blue-500 hover:underline" onClick={()=>navigate(`/admin/blogs/${item._id}`)}>
            Edit
          </button>
        </td>
      </tr>
          ...
          <div className="flex flex-col gap-3 w-full ">
            ...
            <div className="flex flex-col ">
              <div className="flex w-full items-center justify-between mt-8 px-[3rem]">
                ...
                <button className="text-sm bg-[#E97500] px-4 py-2 rounded-xl text-white border-[#E97500] hover:shadow-2xl"
                    ...
                    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                      ...
                      {data.length > 0 && (
                        <tbody>
                          {data.length &&
                            data.map((blog, index) => {
                              return <TableComponent key={index} item={blog} />;
                            })}
                        </tbody>
                      )}
                    </table>
